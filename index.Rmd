...
# for instructors that wish to start at a later section, we will save the imported data.
...
Since lower case `"a"` values occur in the values of the `MHS_setting` column (like outp**a**tient), we want to makes sure that we don't just remove all `"a"` values from the table, just those in the subsequent columns. 
...
Using the `mutate()` function, we specify that we want to change this particular column and replace it with a version of this column that does not cotain characters that are digits, the `r\n` string that we saw, or punctuation marks.
