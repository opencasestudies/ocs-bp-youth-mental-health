...
avocado -Michael:Perhaps "underestimates in the p-values..." is not the correct way to phrase this. I would look for a better way to word this.-Carrie:This is my attempt after Micheal's... open to changes!
...
See [here](https://adv-r.hadley.nz/quasiquotation.html#tidy-dots){target="_blank"} for information about the `:=` colon-equals operator. This operator is more flexible than the normal `=`. It allows expressions on both sides, thus it allows us to use an expression (the values for `new_col`) as the input value to the expressions that follow the `:=` operator.
...
```{r, eval=FALSE, echo=FALSE}
#from Michael - I left this in case we decide we want to include something from this instead
