...
[directlabels](https://tdhock.github.io/directlabels/docs/index.html){target="_blank"}      | to add labels directly to lines in plots  
...
[rstatix](https://cran.r-project.org/web/packages/rstatix/rstatix.pdf){target="_blank"} | to preform proportion test   
...
2. Furthermore, the sampling mechanism utilized for the survey can introduce [selection bias](https://en.wikipedia.org/wiki/Selection_bias){target="_blank"} in cases where the the [sampling methods do not produce a representative sample](https://en.wikipedia.org/wiki/Sampling_%28statistics%29){target="_blank"}. 
...
- [Selectorgadget Tool](https://rvest.tidyverse.org/articles/selectorgadget.html){target="_blank"}
...
Another option for identifying XPaths is to use the [selectorgadget tool](https://rvest.tidyverse.org/articles/selectorgadget.html){target="_blank"}.  
...
+ We extract specific components of the webpage using the `html_elements()` function of the `rvest` package
...
By [piping](https://dplyr.tidyverse.org/articles/dplyr.html#the-pipe){target="_blank"} we mean using the `%>%` pipe operator which is accessible after loading the `tidyverse` or several of the packages within the tidyverse like `dplyr`. This allows us to perform multiple sequential steps on one data input. 
...
Then, we use the `html_elements()` function of the `rvest` package to select just the Table 11.1A element of the webpage.
...
See [this other case study](https://opencasestudies.github.io/ocs-bp-vaping-case-study/){target="_blank"} for more details about how to write a function.
...
If the package does not work for you, alternatively, an RDA file (stands for R data) of the data can be found [here](https://github.com//opencasestudies/ocs-bp-youth-mental-health/tree/master/data/imported) or slightly more directly [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-youth-mental-health/master/data/imported/imported_data.rda). Download this file and then place it in your current working directory within a subdirectory called "imported" within a subdirectory called "data" to copy and paste our code. We used an RStudio project and the [`here` package](https://github.com/jennybc/here_here) to navigate to the file more easily. 
...
We will use `<-` to reassign the result to `table11.1a` after all the steps have been performed.
...
We can use the `mutate()` function along with `across()` to recode these values to `NA`.


```{r}

table11.1a <- table11.1a %>%
   mutate(across(everything(), ~ replace(., . %in% c("nc", "--", "", "*", "dc", "nr"), NA)))
...
# make "nc" values NA etc.
    mutate(across(everything(), ~ replace(., . %in% c("nc", "--", "", "*"), NA))) %>%
...
# make a new variable called type with 9 values that are Specialty followed by 11 values of Nonspecialty
    mutate(type = rep(c(rep("Specialty", 9), rep("Nonspecialty", 11)), length.out = n())) %>%
# make a new variable called subtype with the following values repeated various times
    mutate(subtype = rep(c("Specialty_total",
                       rep("Outpatient", 5),
                       rep("Inpatient", 3),
                       "Nonspecialty_total",
                       rep("Education", 3),
                       ...
                       rep("Juvenile_Justice", 2),
                       rep("Child_Welfare", 2),
                       "combination"), length.out = n())) %>% 
# make a new variable called short_label to use as labels for plots for the data
    mutate(short_label = rep(c(
      "Specialty total", "Outpatient total",
      "Therapist", "Clinic", "Day program",
      "In-home Therapist", "Inpatient total",
      "Hospital", "Residential Center",
      "Nonspecialty total", "School total",
...
table11.2a <- table11.2a %>% mutate(data_type = "Major_Depressive_Episode")
head(table11.2a)

table11.2b <- data_dem_settings(TABLE = table11.2b)
table11.2b <- table11.2b %>% mutate(data_type = "Major_Depressive_Episode")
head(table11.2b)

table11.3a <- data_dem_settings(TABLE = table11.3a)
table11.3a <- table11.3a %>% mutate(data_type = "Severe_Major_Depressive_Episode")
head(table11.3a)

table11.3b <- data_dem_settings(TABLE = table11.3b)
table11.3b <- table11.3b %>% mutate(data_type = "Severe_Major_Depressive_Episode")
head(table11.3b)

table11.4a <- data_dem_settings(TABLE = table11.4a)
table11.4a <- table11.4a %>% mutate(data_type = "Treatment")
head(table11.4a)

table11.4b <- data_dem_settings(TABLE = table11.4b)
table11.4b <- table11.4b %>% mutate(data_type = "Treatment")
head(table11.4b)
```
...
***
<details><summary> Click here for more information about each of the check functions. </summary>
...
Using the definitions from the [Census Bureau](https://www.census.gov/data/academy/resources/one-pagers/race.html){target="_blank"}: 
...
```{r}
percents <- percents %>% mutate(Demographic = str_replace(string = Demographic,
                                             pattern = "AIAN",
...
counts <- counts %>% mutate(Demographic = str_replace(string = Demographic,
                                             pattern = "AIAN",
                                         replacement = "American Indian and Alaska Native"))
...
readr::write_csv(percents, here::here("data", "wrangled", "percents.csv"))
readr::write_csv(counts, here::here("data", "wrangled", "counts.csv"))
readr::write_csv(table11.1a, here::here("data", "wrangled", "table11.1a.csv"))
readr::write_csv(table11.1b, here::here("data", "wrangled", "table11.1b.csv"))
```
...
If the package does not work for you, alternatively, an RDA file (stands for R data) of the data can be found [here](https://github.com//opencasestudies/ocs-bp-youth-mental-health/tree/master/data/wrangled) or slightly more directly [here](https://raw.githubusercontent.com/opencasestudies/ocs-bp-youth-mental-health/master/data/wrangled/wrangled_data.rda). Download this file and then place it in your current working directory within a subdirectory called "wrangled" within a subdirectory called "data" to copy and paste our code. We used an RStudio project and the [`here` package](https://github.com/jennybc/here_here) to navigate to the file more easily. 
...
See [here](https://tdhock.github.io/directlabels/docs/index.html){target="_blank"} for more information about the options for adding labels with this package.  
...
> Pearson's chi-squared test is used to determine whether there is a statistically significant difference between the **expected frequencies** and the **observed frequencies** in one or more **categories** of a [contingency table](https://en.wikipedia.org/wiki/Contingency_table){target="_blank"}.
...
The [degrees of freedom](https://en.wikipedia.org/wiki/Degrees_of_freedom_(statistics)) ($df$) is calculated like so: $df= (r-1)(c-1)$. Note that as according to Wikipedia: "degrees of freedom is the number of values in the final calculation of a statistic that are free to vary" or "the number of independent pieces of information that go into the estimate of a parameter".
...
This [website](https://homepage.divms.uiowa.edu/~mbognar/applets/chisq.html){target="_blank"} has a simple way to check this, without requiring you to get out a ruler and consult this plot. (Note on this website the notation for $df$ is ${\nu}$.)
...
See here for a more thorough explanation of the [chi-squared test](https://www.ling.upenn.edu/~clight/chisquared.htm){target="_blank"}.
...
```{r}
chi_squared_11.2a <- chi_squared_11.2a %>%
...
```{r, echo = FALSE}
knitr::include_graphics(here::here("img", "mainplot_orig.png"))
```
...
Interested in exploring more open case studies? Visit **[opencasestudies.org](https://opencasestudies.org)** to access additional studies and open educational resources.
...
[Cheatsheet on RStuido IDE](https://opensource.posit.co/resources/cheatsheets/rstudio-ide/rstudio-ide.pdf){target="_blank"}  
...
[Sampling methods](https://en.wikipedia.org/wiki/Sampling_%28statistics%29){target="_blank"}   
...
[Piping in R](https://dplyr.tidyverse.org/articles/dplyr.html#the-pipe){target="_blank"}   
[Writing functions](https://r4ds.had.co.nz/functions.html){target="_blank"}
Also see [this case study](https://opencasestudies.github.io/ocs-bp-vaping-case-study/){target="_blank"}  for more information on writing functions.   
...
[Chi-squared distribution](https://en.wikipedia.org/wiki/Chi-squared_test#/media/File:Chi-square_distributionCDF-English.png){target="_blank"}  
[chi-square distribution applet](https://homepage.divms.uiowa.edu/~mbognar/applets/chisq.html){target="_blank"}  
...
[`directlabels` package methods](https://tdhock.github.io/directlabels/docs/index.html){target="_blank"}  
...
We would like to acknowledge Qier Meng and [Michael Breshock](https://mbreshock.github.io/) for their contributions to this case study. 
...
<script type='text/javascript' id='clustrmaps' src='//cdn.clustrmaps.com/map_v2.js?cl=080808&w=a&t=tt&d=V1QZLVYhdPKkJALpt8zp68Hxs1qOXSEVgEylL9gFs9g&co=ffffff&cmo=3acc3a&cmn=ff5353&ct=808080'></script>
