...
AVOCADO: I really like the plot! Should clarify what the bottom two plots are referring to? i.e. % of major depressive episode (vs treatment which is only in the top left plot)? Also, the color legend only is relevant for the bottom right plot. Should we move those closer? 
...
AVOCADO: Was there suppose to be an "A." below? I see a "B." and "C"
...
**Summary**: While depression appear to be on the rise for youths, youths also appear to be seeking more mental health care.

In this case study, we will be using data from the [National Survey on Drug Use and Health (NSDUH)](https://nsduhweb.rti.org/respweb/homepage.cfm){target="_blank"} related to treatment and major depressive episode rate to explore how this have changed over time and how different groups compare. 
...
AVOCADO: should we update the "R especially efficient" text?
...
According to the CDC the rate of suicide has increased for most age groups in the US over the past decade and a half.

AVOCADO: unsure if you meant to show a different plot (i.e. one stratified by age groups) or if you meant to write different text in the sentence above (i.e. stratified by gender)? 
...
While suicide does appear to be increasing among youths it also appears to be increasing among middle aged adults for both females: 
...
AVOCADO: Do we need the plot below? It's not super clear to me how the text above relates as suicide is so far down the list? Also it doesn't have age in the plot? 
...
AVOCADO: oh i see, i think this is the plot you were referring to in the text above. Yeah, I would get ride of the `img/mortality.png` plot and just use this plot. However, the age groups don't quite match up. The pie chart doesn't have a category 35-54? 
...
These data are available in a large PDF with all the results by year. 
However, it is not easy to find this PDF and it would be difficult and time consuming to find our tables of interest and to extract the data from the pdf with `pdftools`. 
Again, if we instead decided to copy paste the data from the website to another file that we would also need to import, this would not be as efficient or reproducible and might result in errors. 
...
The `rvest`  package can be thought of as the `pdftools` package for web scraping. 
Upon pulling the data, additional wrangling will likely be required; but like the `pdftools` package, `rvest` streamlines the extraction process.  
...
AVOCADO: Should we mentioned how all the functions `image_read()` work from magick? it's really cool! Seems like a data science learning objective? 
...
First, we want to remove the last row of our data frame, which happens to be the legend of our table. 
Recall from looking at the website that there is a legend for this table.
...
Let's take a look at the year 2004 column.
```{r}
table11.1a %>%
  dplyr::as_tibble() %>%  
  select(`2004`) %>%
...
If we take a closer look at the first column of our table (using the `pull()` function of the `dplyr` package), we can see that besides the `"a"` values that we see adjacent to our numeric values in the body of the table, we also have some large white spaces, some numeric values, instances of `\r\n`, as well as some commas and other punctuation marks. 
...
We can use the `str_remove_all()` function, which is a variant of the `str_remove()` function of the `stringr` package. 
This allows us to remove all occurrences of specified characters in each row rather than just the first occurrence (which is what `str_remove()` does) to remove the digit values, the `\r\n` characters and the punctuation marks from the column called `MHS_setting`.
...
AVOCADO: these sentences are a bit dense. Could we break them up a bit with code in between? 
...
data_prep_settings <- function(TABLE, new_col, pivot_col){
  ...
  mutate(type = c(rep("Specialty", 9), rep("Nonspecialty", 11))) %>%
  mutate(subtype = c("Specialty_total", rep("Outpatient", 5), 
                     rep("Inpatient", 3), "Nonspecialty_total", 
                     rep("Education", 3), rep("General_medicine", 2),
                    rep("Juvenile_Justice", 2), rep("Child_Welfare", 2), 
                    "combination")) %>%
  mutate(short_label = c("Specialty total", "Outpatient total", "Therapist", "Clinic", "Day program", "In-home Therapist", "Inpatient total", "Hospital", "Residential Center", "Nonspecialty total", "School total", "School Therapist", "School Program", "General Medicine", "Family Dr", "Justice System", "Justice System", "Welfare", "Fostercare", "Specialty Combination")) %>%
...
We can use the `str_replace()` function of the `stringr` package, because now we want to only replace the first instance of `1` with `Age_1`.
...
data_dem_settings <- function(TABLE){
  ...
  mutate(subgroup = c("Total", rep("Age", 4), 
                    rep("Sex", 3), rep("Race/Ethnicity", 9))) %>%
...
We will also indicate that would like to use the `Demographic` variable in our dataset to group our data and to color our data.
...
We also use the `theme_classic()` theme. 
...
To make sure our plot is not too overwhelming, let's limit to only age and sex subgroups. 
...
avocado: the dl.move functions are set up for the rendering the rmarkdown - so if you are viewing the case study from RStudio the labels will overlap.
...
We can also see an increase for both sexes since about 2011, but there is a steeper increase for females. 
Furthermore, females have a much higher rate than males across all years.
...
In the next section, we will formally test whether `Sex` is independent of the differences in rates of MDE across time. 

whether there is a statistically significant difference between the expected frequencies and the observed frequencies in one or more categories of a contingency table.
...
How might we remove the `NHOPI` group from the legend?
...
Now let's take a look at how the rate of youths reporting having a major depressive episode (MDE) **with severe impairment** has changed over time. See the [**What are the data?**] section about how severe impairment was defined. 
...
The data for the AIAN group is sparse, so it is unclear if their levels would be lowest on the last year.

### MDE Treatment

Now we will take a look at those who received treatment. 
First, let's look overall using the `Demographic == "TOTAL"` group.
...
This rate is increasing over time like the trend of those who had a major depressive episode, yet the data are much more variable from one year to the next. 
...
We can see that youths appear to be receiving care in nonspecialty facilities at a slightly higher rate than that of specialty facilities. 
...
In the next plot, we first filter for `Male`, `Female` and `Total` and then facet by the `Demographic` variable. 
...
Thus, we want to test if sex and year are independent. In other words, we want to know if sex influences the number of major depressive episodes in 2004 and 2018. Or stated yet another way: _are major depressive episode counts across the two years associated with sex?_  
...
Consider a hypothetical scenario where we sample 100 individuals and report a contingency table representing the number of hypothetical individuals that we observed who are left and right handed, stratified by males and females.

AVOCOADO: should we include text about how gender is not binary here? 

```{r}
observed <- tibble(Sex = c("Male", "Female", "Total"), 
...
We can see that there are two variables: `Sex` and `Handedness` and each have two levels (`Male` and `Female` for `Sex` and right and left for handedness). 
The table also includes totals of each of the 4 possible groups as well as the overall total.

Next, we create a table of what we **expect** assuming `Sex` and `Handedness` are independent. 
...
```{r}
expected <- tibble(Sex = c("Male", "Female", "Total"), 
...
So to calculate the ${\chi}^2$ test statistics for handedness and sex manually, we can do the following for each of the four values in the table (ignoring the `Totals`) like this:
...
Therefore, using the data we have, we do not have evidence in our data to suggest that sex and handedness are not independent.
...
We see that the $p$-value is very small, which suggests there is an association between `Sex` and the number of major depressive episodes across time (2004 compared to 2018). 
...
We can use the `guides()` function of the `ggplot2` package to modify the legend to specify that we want the legend to be displayed in 2 rows like so `guides(guide_legend(nrow = 2))`.
...
AVOCADO: Maybe we take out Race/Ethnicity in the final plot? Two reasons: (1) there isn't a difference between the groups, (2) it's by far the most complicated plot to interpret and makes the plot a lot harder understand in a first passing. 
Could we also **bold** the subtitles in the top two plots. When I first read the case study, it was hard to catch the difference between the left and right plot (i.e. MDE events vs treatment for MDE)
