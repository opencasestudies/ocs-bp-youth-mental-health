...
# `jhudsl/base_ottr` does not bundle magick. Install it when rendering in a
# fresh container, while leaving local installations untouched.
...
replace_missing_markers <- function(data) {
  data %>%
    dplyr::mutate(
      dplyr::across(
...
```{r, eval=TRUE}
# Packages used in this case study that are not included in the standard
# package list.
install.packages(c(
  "cowplot", "directlabels", "ggthemes", "magick", "OCSdata", "rstatix"
))

# These packages are installed from GitHub. `dependencies = TRUE` also
# installs packages required by wordcountaddin, including koRpus.
...
```{r, out.width = "20%", echo = FALSE, fig.align ="center", eval = knitr::is_html_output()}
...
[directlabels](https://cran.r-project.org/package=directlabels){target="_blank"}      | to add labels directly to lines in plots  
...
2. Furthermore, the sampling mechanism utilized for the survey can introduce [selection bias](https://en.wikipedia.org/wiki/Selection_bias?oldformat=true){target="_blank"} in cases where the the [sampling methods do not produce a representative sample](https://en.wikipedia.org/wiki/Sampling_%28statistics%29){target="_blank"}. 
...
We have also saved the data as it was on this website in this repository to ensure that our case study will remain stable despite possible changes to the source website.

Thus we will proceed with this URL:

```{r}
NSDUH_url <- here::here("data", "raw", "samhsa.gov_2020_tables.htm")
```
...
Using the definitions from the [Census Bureau](https://www.census.gov/topics/population/race/about.html){target="_blank"}: 
...
readr::write_csv(percents, file = here::here("data", "wrangled", "percents.csv"))
readr::write_csv(counts, file = here::here("data", "wrangled", "counts.csv"))
readr::write_csv(table11.1a, file = here::here("data", "wrangled", "table11.1a.csv"))
readr::write_csv(table11.1b, file = here::here("data", "wrangled", "table11.1b.csv"))
```
...
See [here](https://cran.r-project.org/package=directlabels){target="_blank"} for more information about the options for adding labels with this package.  
...
This [chi-square distribution applet](https://mabognar.github.io/apps/chisq.html){target="_blank"} has a simple way to check this, without requiring you to get out a ruler and consult this plot. (On this applet, the notation for $df$ is ${\nu}$.)
...
[Cheatsheet on RStuido IDE](https://opensource.posit.co/resources/cheatsheets/rstudio-ide/){target="_blank"}  
...
[chi-square distribution applet](https://mabognar.github.io/apps/chisq.html){target="_blank"}  
...
[`directlabels` package methods](https://cran.r-project.org/package=directlabels){target="_blank"}  
...
We would like to acknowledge [Qier Meng](https://www.opencasestudies.org/post/2020-11-16-welcome-Qier-Meng-and-Michael-Breshock.html) and [Michael Breshock](https://mbreshock.github.io/) for their contributions to this case study. 
