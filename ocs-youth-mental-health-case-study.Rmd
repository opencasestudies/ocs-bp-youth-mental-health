...
We would also like to remove a couple of rows that are completely empty. These are the rows where the first column values are `Genearl_Medicine` and `Juvenile_Justice`, and `Child Welfare`. If we look at the website, we can see that these were leading line with no data.
...
library(tidyr)
table11.1a %<>%
  tidyr::pivot_longer(cols = - MHS_setting, 
                  names_to = "Year",
                 values_to = "Number") %>%
...
We also plan to replace the first column name with `Demographic` everytime.
...
mutate(Demographic = str_replace(string = Demographic, 
                                pattern = "1", 
                            replacement = "Age_1")) %>%
  mutate(dplyr::across(.cols = -Demographic,
                stringr::str_remove_all, "a|,")) %>%
     mutate(across(-Demographic, as.numeric)) %>%
     filter(rowSums(is.na(select(., is.numeric))) <
              length(select(., is.numeric)))}
...
We use thefunction to wrangle the next pair of tables. 
```{r}
