...
2. Implementation of a chi-squared test for indepence.
3. Interpretation of a chi-squared test for indepence.

<u>**Data science Learning Objectives:**</u>

1. Scrape data directly from a website (`rvest`).
2. Subset and filter data (`dplyr`).
3. Write functions to wrangle data repetively.
...
**avocado will update at the end**
...
While suicide does appear to be increasing amoung youths it also appears to be increasing amoung middle aged adults for both females and males. 
...
**So although sucide is on the rise for most age groups, sucide is one of the top *two* contributors to death for youths.**   

Thus this warrents further examination of the mental health of American youths.  
...
**Michael::Perhaps "underestimates in the p-values..." is not the correct way to phrase this. I would look for a better way to word this.** Carrie: This is my attempt after Michael's... open to changes!
...
This survey started in 1971 and is conducted annualy in all 50 states and the District of Columbia. Approximately 70,000 people (age 12 and up) are interviewed each year about health realted issues. Only civilian, noninstitutionalized individuals are included. Households are randomly selected and than a professional interviewer visists the addresses and asks one or two of the residents to inverview. The interviewer brings a laptop with them that the participants use to fill out the survey which typically takes an hour to complete. If a participant chooses to particpate they receive $30 in cash. All collected information is confidential and is used for disease surveillance and to guide public policy particuarlly focused on drug and alcohol use as well as mental health. See [here](https://nsduhweb.rti.org/respweb/about_nsduh.html){target="_blank"} for more details about the survey.
...
We also want to replace the spaces with a single space. We can see that sometimes there appears to be more than one space. We can specify that we want any occurance of 1 or more  to be replaced by using the `{1,}` notation.
...
We can remove the empty rows using the `filter()` function of the `dplyr` package. We can specify that we dont want to keep these rows by using the `!=` not equal to opperator. 
...
We will also want to only wrangle the data up to the point that we cahnge the shape of the data, so that we can check how the data looks first.
...
This will allow us to compare if the frequencines of major depressive episodes differs from what we would expect by chance if the variables years and sex were independent.

The null hypothesis is that the variables are independent or that the  difference in the proportions is equal to zero. Thus we want to test if sex and year are independent. In otherwords, we want to know if sex influences the number of major depressive episodes in 2004 and 2018. Or stated yet another way, Are major dperessive episode counts across the two years associated with sex.  
...
Thus, to conduct this statistical test, we need to produce a [contingency table](https://en.wikipedia.org/wiki/Contingency_table){target="_blank"}, which in this case could also be called a 2x2 table, which is the simplest kind of contigency table (only two levels for two variables).
...
The total righthanded is 88 out of 100 which is equal to 88%.
The total lefthanded is 12  out of 100 which is equal to 12%.

Thus if each sex had the same amount of righthandedness and lefthandedness, we would expect 88% of the males to be righthanded and 12 % to be lefthanded; and
we would expect the exact same proportions of 88% righthanded and 12% lefhanded for the females.

Since we have 50 males and 50 females and 12% of 50 is 6 and 88% of 50 is 44, we would thus have the following table of expected freauencies:
...
the jth cell of a contgency table with $m$ cells.
...
${\chi}^2$ = fraction of the square difference of the observed minus the expected, divided by expected for righthanded males + the same for lefthanded males + the same for righthanded females + the same for lefthanded females
...
Now let's create a contigency table with our own data and see how we can implement this test in R.
...
The resulting object contains all the values we need for out contigency table. 
...
A contingency table can  now be produced from this data which is in long format by transforming the data to wide format and repurposing some values as row names. To reformat the data to wide format we can use the `pivot_wider()` function of the `tidyr` package.

For this function there are several impartant arguments:  
...
Now we can use the `column_to_rownames()` function of the `tibble` package to make the `Demographic` variable levels the rownames. Otherewise we would need to select the numeric columns to perform the stats test.
...
Note that a contigency table would also have totals for all groups as well, but this is not necessary for the `stats::chisq.test()` function.
...
We will start by taking a look at where youths are recieving treatment by using values from table11.1b which has the percentage values for counts presented in table11.1a.


```{r}
plotMHS <-table11.1b %>%
  filter(stringr::str_detect(short_label, 'total') ) %>%
  ggplot2::ggplot(aes(x = Year, y = Percent, group = MHS_setting, color = short_label)) +
  ggplot2::geom_line() +
  facet_wrap(~type)+
...
directlabels::direct.label(plotMHS, list("angled.boxes", cex = .8))
```
We can see that youths appear to be receiving care in nonspeciatly facilities at a slightly higher rate than that of specialty facilities. However, the rates appear to be very similar and the relative differences appear to be consistent across time.
...
plotMHS <-table11.1b %>%
  ...
  ggplot2::geom_line() +
  facet_wrap(~type)+
  ggplot2::scale_x_continuous(breaks = seq(2000, 2019, by=1),
...
Let's try another way of labeling our lines:
...
plotMHS <-table11.1b %>%
  ...
  ggplot2::geom_line() +
  facet_wrap(~type)+
  ggplot2::scale_x_continuous(breaks = seq(2002, 2019, by=1),
...
direct.label(plotMHS, list(dl.trans(y = y +0.3),"far.from.others.borders", dl.move("School Therapist", 2010, 10), dl.move("Fostercare", 2010, 1)))
...
Now we will take a look at the rate of major depressive episodes amoung youths over time of the various demographic groups.

```{r}
MDE <-percents %>%
  filter(data_type == "Major_Depressive_Episode")%>%
  filter(subgroup != "Race") %>%
...
directlabels::direct.label(MDE, list(dl.trans(y = y +0.35), "far.from.others.borders", cex = .8))
```
...
There is only one value for NHOPI, so lets remove this from the plot:
...
directlabels::direct.label(Age_episode, list("angled.boxes", cex = .8))
...
directlabels::direct.label(Sex_episode, list("angled.boxes", cex = .8))
...
directlabels::direct.label(MDES, list(dl.trans(y = y +0.35), "far.from.others.borders", cex = .8))
...
directlabels::direct.label(Race_episode, list(dl.trans(y = y +0.3),"far.from.others.borders", cex = .8))
...
directlabels::direct.label(treat, list("angled.boxes", cex = .8))
...
#directlabels::direct.label(Race_episode, list("far.from.others.borders", cex = .8))
Race_episode

```


Ok so now we know how the rates of different subgroups compare for having a major depressive episode in the past year, having a major depressive episode
with severe impairment, and recieving treatment. But how do the rates of these there things compare for each group?


```{r}
sex_outcomes <-percents %>%
  filter(subgroup %in% c("Sex", "Total")) %>%
  #filter(Demographic != "NHOPI") %>%
  ggplot(aes(x = Year, y = Percent, color = Demographic)) +
  geom_line(aes(linetype= data_type)) +
  scale_x_continuous(breaks = seq(2004, 2018, by=1),
                     labels = seq(2004, 2018, by=1),
                     limits = c(2004, 2018)) +
  labs(title = "Major Depressive Episode in Past Year\namong Persons Aged 12 to 17",
       subtitle = "By Demographic Characteristics, Percentages, 2004-2018") +
  facet_wrap(subgroup~Demographic, strip.position = "top")+
  theme_linedraw()+
...
race_outcomes <-percents %>%
  ...
  labs(title = "Major Depressive Episode in Past Year\namong Persons Aged 12 to 17",
       subtitle = "By Demographic Characteristics, Percentages, 2004-2018") +
  facet_wrap(~Demographic, strip.position = "top", nrow = 3)+
  theme_linedraw()+
  theme(strip.text = element_text(size = 8, face = "bold"),
        axis.text.x = element_text(angle = 90),
        legend.title = element_blank())+
