...
2) Furthermore, the sampling mechanism utilized can introduce [selection bias](https://en.wikipedia.org/wiki/Selection_bias?oldformat=true) in cases where the the [sampling methods do not produce a representative sample](https://en.wikipedia.org/wiki/Sampling_(statistics)?oldformat=true). 

3) Data is collected from human participants; this presents the *potential* for information bias, as there is the *potential* that partificipants in the [sampling frame](https://en.wikipedia.org/wiki/Sampling_frame?oldformat=true) may for a variety of reasons report inaccurate information. 

## **What are the data?**
*** 

The data comes from the [National Survey on Drug Use and Health (NSDUH)](https://nsduhweb.rti.org/respweb/homepage.cfm). 

Every year, thousands are surveyed in 50 states.

This information is used for disease surveillance and to guide public policy. 

This data is made available publicly online] on the [Substance Abuse & Mental Health Data Archive](https://datafiles.samhsa.gov/). 
...
Data is often made available online 

Usually, the data we are interested in is made available on the page as a delimited text file. 

However, sometimes data is not made available in this manner.

How do we proceed in this scenario?

We can manually copy each cell of data. This process is often inefficient, subject to error, and not reproducible. 

We can also use `R` for web scraping. 

[Web scraping](https://en.wikipedia.org/wiki/Web_scraping?oldformat=true) is the process of extracting data from a website.
...
[RStudio](https://rstudio-pubs-static.s3.amazonaws.com/266430_f3fd4660b2744751ab144aa130768a06.html)

[Blog](http://blog.corynissen.com/2015/01/using-rvest-to-scrape-html-table.html)
